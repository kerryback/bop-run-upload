"""GS with a STATE-DEPENDENT price of risk: gamma(x), the minimal stand-in for the
Epstein-Zin/GE kernel GS21 replaced with a constant gamma_x = 0.5.

    gamma(x) = clip(gamma_x - gs_gamma_slope * x / sd(x), gs_gamma_lo, gs_gamma_hi)

WHY THIS FILE EXISTS, rather than a switch inside gs_solve_reg.py. solve_id digests the
solver's own source, so ANY edit to gs_solve_reg.py changes all five bx7 solve_ids --
five solves that cost ~5 h each on Sol and were verified against their precommitment on
2026-09-07. A separate module leaves those intact. The cost is duplication, tracked by
tests/test_gs_solver_parity.py, which asserts the two files differ ONLY in the gamma
block and its parameters.

RECONSTRUCTION, not invention. This economy is `sol_g28` from variants/REPORT.md:553,
ranked 3rd of 24 on realized gap (+0.038, t 17.9) in the grid that PLAN 0.0 rests on,
with room ~= 0. Its code was never in this repo. It matters because the gap is NOT
complexity: REPORT.md 13g found RFF-3600 "needs its heaviest ridge and still trails
lin_rank" in population, yet the ESTIMATED linrank lands 0.039 below its own ceiling
while RFF attains its. That is a second lever -- estimation efficiency, not nonlinearity.

sd(x) is the STATIONARY sd sqrt(sigma_x**2/(1-rho_x**2)) = 0.038431, which is what
tauchen spans +/-4 of. It is NOT xgrid.std(), the sd of uniformly spaced grid points,
which is 2.32x larger; standardising by that would silently give 2.32x less gamma
variation than intended. At slope 0.28 gamma has mean 0.502 and sd 0.258 under the
stationary distribution, saturating 9.1% of the TIME (58% of grid points, which sit far
out in the tails where the density is thin).

At gs_gamma_slope = 0 this file reproduces gs_solve_reg.py exactly: clip(gamma_x, lo, hi)
is gamma_x for the default bounds, so the kernel is unchanged element for element.

usage: python gs_solve_gam.py [xnum] [tol] [outdir]     (params via GS_PARAM_OVERRIDES)
"""
import numpy as np
...
gs_bx = 1.0
gs_ashift = 0.0
# state-dependent price of risk; slope 0 recovers the constant-gamma solver exactly
gs_gamma_slope = 0.0
gs_gamma_lo = 0.05
gs_gamma_hi = 1.0
...
_snap = solstamp.snapshot(
    ...
    [os.path.join(HERE, "gs_solve_gam.py")],
    model="gs_bx",
    skip=("outdir", "HERE", "ov"),
...
#
# gamma now depends on the CURRENT x, so it is a COLUMN vector over the current-x axis
# (rows) rather than a scalar. sd_x is the STATIONARY sd -- see the module docstring for
# why xgrid.std() would be wrong by 2.32x.
#
# The renormalisation is unchanged and still runs row-wise (axis=1), so E[M|x] = e^{-r}
# holds in every state. That is what keeps gamma(x) a change in the PRICE OF RISK and
# not a backdoor change in the riskless rate: the conditional mean of the kernel is
# pinned at e^{-r} for every x, only its covariance with the shock moves.
sd_x = np.sqrt(sigma_x ** 2 / (1 - rho_x ** 2))
gam_x = np.clip(gamma_x - gs_gamma_slope * xgrid / sd_x, gs_gamma_lo, gs_gamma_hi)
gam_s = gamma_x * gmreg          # regime multiplier retained; gmreg=[1,1] leaves it inert
Mx_s, MP_s = [], []
for s in (0, 1):
    g_cur = (gam_x * gmreg[s])[:, None]                  # (xnum, 1), current-state gamma
    M = np.exp(-r - 0.5 * g_cur ** 2
               - g_cur * (xgrid[None, :] - (1 - rho_x) * x_bar - rho_x * xgrid[:, None]) / sigma_x)
    M = M * (np.exp(-r) / (M * pr_x).sum(axis=1))[:, None]
    Mx_s.append(M); MP_s.append(M * pr_x)
print(f"gamma(x): slope {gs_gamma_slope}, range {gam_x.min():.4f}-{gam_x.max():.4f}, "
      f"sd_x {sd_x:.6f}; regime mult {gmreg}; switch probs {psw}", flush=True)
...
np.savez_compressed(os.path.join(outdir, "solution.npz"),
                    ...
                    kappa_e=kappa_e,
                    # like kappa_e: appended as named keys, never into `params`, which
                    # gs_sim_bx.py unpacks positionally as a 15-tuple.
                    gs_gamma_slope=gs_gamma_slope, gs_gamma_lo=gs_gamma_lo,
                    gs_gamma_hi=gs_gamma_hi, gamma_x_grid=gam_x,
