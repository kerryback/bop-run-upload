...
rows = []
for f in sorted(glob.glob(os.path.join(E1, "*_estimators_*_summary.csv"))):
    ...
    rows.append(dict(model=m["model"], tag=m["tag"], seed=int(m["seed"]), window=int(m["w"]),
                     ...
                     orig_drift=drift, prov_e1=e["prov"].iloc[0] if "prov" in e else None))
d = pd.DataFrame(rows)
d["gap"] = d.dkkm - d.lin
...
for (model, tag, w), x in d.groupby(["model", "tag", "window"], sort=False):
    ...
    r = dict(model=model, tag=tag, window=w, n=len(x), gap=x.gap.mean(), fair_gap=x.fair_gap.mean(),
...
cols = ["tag", "n", "gap", "fair_gap", "fair_gap_sd", "fair_gap_t", "seeds_fair_gap_positive", "dkkm", "lin", "fair_lin",
        "mkt_est", "ew", "fair_gap_over_fair_lin", "preregistered", "verdict", "fair_winners"]
print(econ[[c for c in cols if c in econ]].round(4).to_string(index=False))
print(f"\noriginal four methods, E1 vs committed: max |diff| {d.orig_drift.max():.2e} over {len(d)} seeds")
