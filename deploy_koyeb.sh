...
# Create the service
# The run command:
#   1. Sends init message to monitor (creates log file immediately)
#   2. Starts a background log streamer (sends logs to monitor every 60s)
#   3. Runs main.py with output tee'd to run.log
#   4. Kills the background streamer when done
#   5. Submits final logs to monitor
#   6. Sends kill request to terminate the app
...
koyeb services create worker \
  ...
  --git-run-command "python -c \"import requests, os, datetime; requests.post(os.environ['MONITOR_URL']+'/init-logs', json={'app_name': os.environ['KOYEB_APP_NAME'], 'model': '$MODEL', 'start': $START, 'end': $END, 'instance_type': '$INSTANCE_TYPE', 'started_at': datetime.datetime.now().isoformat()})\"; python utils/log_streamer.py & LOG_PID=\$!; python main.py $MODEL $START $END --koyeb 2>&1 | tee run.log; kill \$LOG_PID 2>/dev/null; python -c \"import requests, os; logs=open('run.log').read() if os.path.exists('run.log') else ''; requests.post(os.environ['MONITOR_URL']+'/submit-logs', json={'app_name': os.environ['KOYEB_APP_NAME'], 'logs': logs})\"; curl -s -X POST \$MONITOR_URL/kill -H \"Content-Type: application/json\" -d \"{\\\"app_name\\\": \\\"\$KOYEB_APP_NAME\\\"}\"" \
...
echo "  2. Initialize log file at monitor (creates link immediately)"
echo "  3. Start background log streamer (sends logs every 60s)"
echo "  4. Run: python main.py $MODEL $START $END --koyeb"
echo "  5. Upload results to s3://$S3_BUCKET/koyeb-results/$WORKFLOW_ID/"
echo "  6. Submit final logs to the monitor"
echo "  7. Auto-delete the app to stop billing"
echo ""
