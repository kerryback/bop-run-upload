<!-- HISTORICAL RECORD -- NOT CITABLE BY ANY LIVE DOCUMENT -->

> **This file is history, not evidence.** Moved here from `variants/` on 2026-09-17.
>
> - **21 of the 24 economies below have no code in this repository, and never did.** Verified: no
>   commit reachable from `--all` has ever held `kp_gam`, `kp_gamy`, `kp_bx`, `kp_dis`, `bgn_types`,
>   `bgn_dis`, `bgn_gamr`, `gs_fixed` or `gs_dis`. The curation happened while importing from an
>   external tree at `bba735f`, which ADDS 130 files and deletes none. Nothing here can be rebuilt by
>   a `git show`.
> - **Every figure is a single seed.** Cross-seed sd of a gap runs 15% to over 300% of its mean on the
>   economies that were later measured properly, so no ordering below the top of any table here is
>   identified.
> - **The KP14 levels ran at growth-option arrival rate 1.72 instead of 1**, and the **GS21 levels ran
>   on three wrong Table I parameters**. Both were corrected afterwards. The GS21 numbers in sections
>   11 and 12 also predate the exact-kernel renormalisation; section 13g carries the re-based table.
> - Its mechanism REASONING is why it was kept: it is the only written record of why eighteen routes
>   were abandoned. Its NUMBERS are not comparable to anything current.
>
> **Live results: `docs/RESULTS.md`.** Which of these economies the current code could still express:
> that document's "Routes with no code" appendix.
