# Working doc — the engineering journal

**What this file is.** A dated journal of how this repository came to work the way it does. One
section per working episode, appended, never rewritten: what was attempted, what was measured, what
broke, and the decision taken. Most sections are the origin story for a rule now enforced by a test
— §17 the single-builder lock, §26 the non-reproducible `solve_id`, §38 an AST-based id measured and
rejected, §62 the measurement protocol, §63 the two pricing defects, §64 the campaign that withdrew
the headline.

**It is not a plan and not results.** What to run next is `docs/NEXTUP.md`; the numbers are
`docs/RESULTS.md`; how the compute was run and what it cost is `docs/RUNS.md`.

**If a session dies, read the LAST section first** — §64 as of 2026-09-22 — then the two or three
before it. Section numbers are a stable address: eight places in `variants/` and `tests/` cite them
(§40, §42, §43, §44, §50), so sections are never renumbered.

---

## §0–§16 — the `variants` → `main.py` refactor: CLOSED

Started 2026-09-04 with a different goal from the one the repo ended up with: absorb the `variants/`
experiments into the main `run_bop_job.sh` → `main.py` workflow, reconcile the three models' solution
methods, and build durable spec-and-summary storage. Its scoping (§1–§6), design phase (§9), eight
answered decisions (§11), Phase 0 table rebuilds (§12, §14) and provenance groundwork (§15, §16) all
completed, and the last two outlived the project: `solstamp` content-addressing and the staged solve
ids are §15 and §16, and they are still what the repo runs on.

**The merge into `main.py` never happened, and should not now.** Everything since §17 has been built
in `variants/`, which is where the protocol, the specs, the registry and all thirteen economies live;
`main.py` is the legacy pipeline and `config.py` its parameter source, kept only for the comparisons
`tests/test_config_parity.py` still makes. The §13 task checklist that this file used to tell a
dying session to resume from is therefore retired: its first unchecked box, "Phase 1 — seeded oracle
array, first cluster run", completed on 2026-09-07 at §27 and has been superseded by three campaigns
since.

The full text of §0–§16, including the session log and the eight decisions, is in git history
(`git log -p -- docs/refactor/WORKING.md`). Nothing below depends on it.
